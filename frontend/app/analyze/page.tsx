...
interface ResumeParameter {
  ...
  weight: number;
  feedback: string;
  status: "good" | "improve" | "critical" | string;
}

interface RewrittenSections {
  summary: string;
  one_weak_bullet_rewrite: string;
}
...
interface ResumeAnalysisResponse {
  ...
  parameters: ResumeParameter[];
  top_3_critical_issues: string[];
...
function getScoreColor(score: number) {
  return score >= 80 ? B.green : score >= 60 ? B.yellow : B.red;
}

function buildSections(analysis: ResumeAnalysisResponse | null): DisplaySection[] {
  if (!analysis) return SECTIONS;

  return analysis.parameters.map((parameter) => {
    const percentScore = Math.max(0, Math.min(100, Math.round(parameter.score * 10)));
    return {
      label: parameter.name,
      score: percentScore,
      color: getScoreColor(percentScore),
      feedback: parameter.feedback,
    };
  });
}

function buildSuggestions(analysis: ResumeAnalysisResponse | null): DisplaySuggestion[] {
  if (!analysis) return SUGGESTIONS;

  const criticalIssues = analysis.top_3_critical_issues.map((issue, index) => ({
    icon: "!",
    title: `Critical issue ${index + 1}`,
    body: issue,
    accent: [B.coral, B.orange, B.amber][index] ?? B.coral,
  }));
  ...
  return [
    ...criticalIssues,
    {
      ...
      body: analysis.rewritten_sections.summary,
      accent: B.navyMid,
    },
    {
      icon: "B",
      title: "Bullet rewrite",
      body: analysis.rewritten_sections.one_weak_bullet_rewrite,
      accent: B.green,
    },
...
export default function AnalyzePage() {
  ...
  const handleFile = useCallback((file: File) => {
    ...
    if (ext !== "pdf") {
      setApiError("Please upload a PDF resume.");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setApiError("Upload a PDF up to 5MB.");
      return;
    }
  ...
  const handleAnalyze = async () => {
    if (!role.trim()) {
      setApiError("Target role is required.");
      return;
    }

    if (!selectedFile) {
      setApiError("Please upload a PDF resume first.");
      return;
    }
    ...
    formData.append("job_target", role.trim());
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(getApiErrorMessage(responseBody, "Resume analysis failed. Please try again."));
      }

      setAnalysis(responseBody as ResumeAnalysisResponse);
      setStage("results");
    } catch (error) {
      setApiError(error instanceof Error ? error.message : "Resume analysis failed. Please try again.");
      setStage("ready");
    }
  };
  ...
  const missingKeywords = analysis?.ats_keywords_missing ?? KEYWORDS.filter((keyword) => !keyword.matched).map((keyword) => keyword.word);
  const keywordTotal = analysis ? Math.max(analysis.ats_keywords_missing.length, 1) : KEYWORDS.length;
  const keywordMatched = analysis ? 0 : KEYWORDS.filter((keyword) => keyword.matched).length;
  const verdictTitle = analysis ? `${analysis.grade} Grade Resume` : "Strong ATS Match";
  const verdictBody = analysis?.overall_verdict ?? `Your resume scores well for ${role}. A few targeted improvements could push you into the top 10% of applicants.`;
  const overallScore = analysis?.overall_score ?? 84;
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      {[
                        { label: "ATS Score", val: `${overallScore}/100`, color: getScoreColor(overallScore)  },
                        { label: "Missing",  val: `${missingKeywords.length}`,   color: missingKeywords.length ? B.yellow : B.green },
                        { label: "Grade",    val: analysis?.grade ?? "Pass",   color: B.orange },
                      ].map(stat => (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                      ...
                      <span style={{
                        ...
                        {analysis ? `${missingKeywords.length} missing` : `${keywordMatched} / ${keywordTotal}`}
                      </span>
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {analysis
                        ? missingKeywords.map((word) => (
                            <span key={word} className="kw-pill-miss">
                              ✕ {word}
                            </span>
                          ))
